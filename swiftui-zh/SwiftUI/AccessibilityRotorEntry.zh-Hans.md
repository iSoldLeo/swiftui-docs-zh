---
来源： https://developer.apple.com/documentation/SwiftUI/AccessibilityRotorEntry
...
辅助功能旋转器是辅助功能用户快速导航到用户界面特定元素以及这些元素中特定文本范围的快捷方式。

旋转器中的条目可包含一个标签，用于向用户标识该条目，还可包含一个标识符，用于确定旋转器条目应导航到哪个辅助功能元素，以及一个可选范围，用于导航到相关辅助功能元素文本中的特定位置。条目还可以指定一个处理程序，在导航到该条目之前调用，以完成将辅助功能元素显示在屏幕上所需的手动操作。

在下面的示例中，一个信息应用程序创建了一个旋转器，允许用户专门导航到来自 VIP 的信息。
...
在 ForEach 迭代中有多个辅助功能元素或没有`ScrollView` 的情况下，也可使用可选命名空间创建条目。在这种情况下，可能需要使用`prepare` 闭包，以便使用`ScrollViewReader` 将元素滚动定位。调用`accessibilityRotorEntry(id:in:)` 时也应传递相同的命名空间，以标记与此条目相关的辅助功能元素。

在下面的示例中，一个消息应用程序创建了一个旋转器，允许用户专门浏览来自 VIP 的消息。旋转条目与信息内容文本相关联，而信息内容文本是 ForEach 中生成辅助功能元素的两个视图之一。该视图被标记为`accessibilityRotorEntry(id:in:)`，以便能被`AccessibilityRotorEntry`找到，而`ScrollViewReader`则与`prepare`闭合一起使用，以将其滚动定位。
...
- **init(_:textRange:prepare:)**：创建具有特定标签和范围的转子条目。该转子条目将与拥有该转子的辅助功能元素相关联。
- **init(_:id:textRange:prepare:)**：创建具有特定标签和标识符的 Rotor 条目，可选择范围。

### 在命名空间中创建一个已标识的转子条目

- **init(_:id:in:textRange:prepare:)**：创建具有特定标签、标识符和命名空间的转子条目，可选择范围。
- **init(_:_:in:textRange:prepare:)**：创建具有特定标签、标识符和命名空间以及可选范围的 Rotor 条目。
...
- **init(_:_:in:textRange:prepare:)**: Create a Rotor entry with a specific label, identifier and namespace, and with an optional range.
