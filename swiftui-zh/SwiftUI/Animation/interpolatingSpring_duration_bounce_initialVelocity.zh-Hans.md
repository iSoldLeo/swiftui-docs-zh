---
来源：https://developer.apple.com/documentation/SwiftUI/Animation/interpolatingSpring(持续时间：反弹：初始速度：)
抓取时间：2025-12-01T11:01:26Z
---
...
内插弹簧动画使用阻尼弹簧模型在[0, 1]范围内产生数值，然后在动画属性的[from, to]范围内进行内插。通过添加每个动画的效果，在重叠动画中保持速度。
...
- **initialVelocity**：弹簧的初始速度，取值范围为 [0，1]，代表动画值的大小。
...
- **spring**：持久的弹簧动画。当与同一属性上的`spring()` 或 `interactiveSpring()` 动画混合时，每个动画都将被其后续动画取代，并保持从一个动画到下一个动画的速度。可选择在一段时间内混合弹簧之间的响应值。
