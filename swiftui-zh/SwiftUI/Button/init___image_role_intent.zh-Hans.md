--- 来源：https://developer.apple.com/documentation/SwiftUI/Button/init(_:image:role:intent:)

抓取时间：2025-12-02T21:48:09Z
...
创建一个具有指定角色的按钮，该按钮的标签由字符串和图像资源生成。
...
- **init(_:role:intent:)**：创建一个具有指定角色的按钮，该按钮执行 `AppIntent` 操作，并根据字符串生成其标签。

- **init(role:intent:label:)**：创建一个具有指定角色的按钮，该按钮执行 `AppIntent` 操作。

- **init(_:systemImage:role:intent:)**：创建一个具有指定角色的按钮，该按钮根据字符串和系统图像生成其标签。


---
source: https://developer.apple.com/documentation/SwiftUI/Button/init(_:image:role:intent:)
crawled: 2025-12-02T21:48:09Z
---
