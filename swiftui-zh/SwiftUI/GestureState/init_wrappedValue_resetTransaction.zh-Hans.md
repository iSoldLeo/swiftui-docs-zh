---
来源：https://developer.apple.com/documentation/SwiftUI/GestureState/init(wrappedValue:resetTransaction:)
...
创建一个视图状态，该状态源于一个手势，并带有一个包裹状态值和一个重置状态值的事务。
...
- **init(wrappedValue:reset:)**：创建一个从手势派生的视图状态，该视图状态带有一个包裹状态值和一个闭包，闭包提供了一个重置状态值的事务。
