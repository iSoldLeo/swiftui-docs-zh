---
来源：https://developer.apple.com/documentation/SwiftUI/InsettableShape/strokeBorder(style:antialiased:)
抓取时间：2025-12-03T05:37:34Z
---
...
返回一个视图，该视图是`self`内嵌`style.lineWidth / 2`，用`style`描边，然后填充前景色的结果。
...
- **strokeBorder(_:lineWidth:antialiased:)**：返回一个视图，该视图是用`content`填充`self`的`lineWidth`大小的边框（又称内描边）的结果。这等同于用`lineWidth / 2`嵌入`self`，并用`lineWidth`作为线宽描边得到的形状。
- **strokeBorder(lineWidth:antialiased:)**：用前景色填充`lineWidth` 大小的边框（又称内描边）后返回的视图。这相当于用`lineWidth / 2`嵌入`self`，并用`lineWidth`作为线宽描边。
- **strokeBorder(_:style:antialiased:)**：返回用`style.lineWidth / 2`嵌入`self`，用`style`描边，然后用`content`填充后得到的视图。


---
source: https://developer.apple.com/documentation/SwiftUI/InsettableShape/strokeBorder(style:antialiased:)
crawled: 2025-12-03T05:37:34Z
---
