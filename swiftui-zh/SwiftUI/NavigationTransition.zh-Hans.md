---
来源： https://developer.apple.com/documentation/SwiftUI/NavigationTransition
抓取时间： 2025-12-02T17:34:27Z
---
...
- **automatic**：自动为当前上下文选择合适的演示过渡的样式。
- **zoom(sourceID:in:)**：从给定源视图缩放显示视图的导航转换。

## 支持类型

- **AutomaticNavigationTransition**：一种样式，可根据当前上下文自动选择合适的演示转换。
- **ZoomNavigationTransition**：从给定源视图缩放显示视图的导航转换。使用`View/matchedTransitionSource(id:namespace:)`修饰符指明源视图。
...
- **matchedTransitionSource(id:in:)**：设置该视图的导航过渡样式：将此视图标识为导航转换（如缩放转换）的源。
...
- **zoom(sourceID:in:)**: A navigation transition that zooms the appearing view from a given source view.

## Supporting Types

- **AutomaticNavigationTransition**: A style that automatically chooses the appropriate presentation transition for the current context.
