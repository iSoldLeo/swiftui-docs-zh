---
来源：https://developer.apple.com/documentation/SwiftUI/Path/addArc(center:radius:startAngle:endAngle:clockwise:transform:)
...
- **endAngle**：到圆弧终点的角度，从 x 轴正向量起。
- **clockwise**：为真，表示顺时针方向的圆弧；为假，表示逆时针方向的圆弧。
- **transform**：在添加到路径之前应用于弧的仿射变换。如果未指定，默认为身份变换。

## 讨论

此方法使用指定的半径和角度计算起点和终点，使用一系列三次贝塞尔曲线逼近这些点之间的一段圆，然后将这些曲线添加到路径中。

`clockwise`参数决定创建弧线的方向；最终路径的实际方向取决于`transform`参数和绘制路径的上下文的当前变换。不过，由于 SwiftUI 默认使用垂直翻转坐标系（原点位于视图的左上方），因此指定顺时针方向的弧线会在应用变换后产生逆时针方向的弧线。
...
- **addEllipse(in:transform:)**：将适合指定矩形的椭圆添加到路径。
