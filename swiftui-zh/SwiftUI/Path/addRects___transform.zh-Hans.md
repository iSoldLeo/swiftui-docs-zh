---
来源： https://developer.apple.com/documentation/SwiftUI/Path/addRects(_:transform:)
...
调用此方便方法相当于为数组中的每个矩形重复调用`addRect(_:transform:)` 方法。

- 参数：

- rects：以用户空间坐标指定的矩形数组。
- transform：变换：在添加到路径之前应用于椭圆的仿射变换。如果未指定，默认为身份变换。
