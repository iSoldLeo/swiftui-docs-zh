---
来源：https://developer.apple.com/documentation/SwiftUI/Path/subtracting(_:eoFill:)
抓取时间： 2025-12-02T21:42:41Z
---
...
- **other**：要减去的路径。
- **eoFill**：是否使用偶数规则（如果为 true）或非零规则（如果为 false）来确定哪些区域应作为路径的内部区域。
...
生成路径的填充区域就是这条路径的填充区域，并移除填充区域`other`。
