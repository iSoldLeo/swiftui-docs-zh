---
来源：https://developer.apple.com/documentation/SwiftUI/Path/union(_:eoFill:)
抓取时间： 2025-12-01T02:33:36Z
---
...
生成路径的填充区域是两条路径的填充区域相加的结果。

两条路径中任何一条未封闭的子路径都被假定为封闭路径。使用偶数填充规则或非零填充规则填充这条路径的结果是相同的。
...
- **subtracting(_:eoFill:)**：返回一条新路径，并填充此路径中不在给定路径中的区域。
