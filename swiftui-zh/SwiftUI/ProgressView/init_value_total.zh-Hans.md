--- 来源：https://developer.apple.com/documentation/SwiftUI/ProgressView/init(value:total:)

抓取时间：2025-12-03T05:43:44Z
---
...
如果该值不是 `nil`，但超出 `0.0` 到 `total` 的范围，进度视图会将该值限制在这些范围内，并四舍五入到最接近的边界值。`nil` 表示进度不确定，在这种情况下，进度视图会忽略 `total`。
