--- 来源：https://developer.apple.com/documentation/SwiftUI/Shader/Argument/boundingRect
抓取时间：2025-12-04T13:29:16Z
...
返回一个参数值，表示着色器所附加的形状或视图的边界矩形，例如 `float4(x, y, width, height)`。对于没有自然边界矩形的着色器（例如，绘制到 `GraphicsContext` 中的滤镜效果），此值为未定义。
...
- **colorArray(_:)**：返回由提供的颜色值数组定义的参数值。当传递给 MSL 函数时，它将转换为 `device const half4 *ptr, int count` 参数对。

- **data(_:)**：返回由提供的数据值定义的参数值。当传递给 MSL 函数时，它将转换为 `device const void *ptr, int size_in_bytes` 参数对。
