--- 来源：https://developer.apple.com/documentation/SwiftUI/Shader/Argument/color(_:)

抓取时间：2025-12-04T13:29:17Z
...
返回一个表示 `color` 的参数值。当传递给 MSL 函数时，它将转换为 `half4` 值，即目标颜色空间中的预乘颜色。
...
- **boundingRect**：返回一个表示着色器所附加的形状或视图的边界矩形的参数值，即 `float4(x, y, width, height)`。对于没有自然边界矩形的着色器（例如，绘制到 `GraphicsContext` 中的滤镜效果），此值未定义。

- **colorArray(_:)**：返回由提供的颜色值数组定义的参数值。传递给 MSL 函数时，它将转换为 `device const half4 *ptr, int count` 参数对。

- **data(_:)**：返回由提供的数据值定义的参数值。传递给 MSL 函数时，它将转换为 `device const void *ptr, int size_in_bytes` 参数对。

- **float(_:)**：返回表示 MSL 值 `float(x)` 的参数值。

- **float2(_:)**：返回表示 MSL 值 `float2(point.x, point.y)` 的参数值。
