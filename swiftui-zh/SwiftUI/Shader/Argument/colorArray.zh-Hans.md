--- 来源：https://developer.apple.com/documentation/SwiftUI/Shader/Argument/colorArray(_:)

抓取时间：2025-12-04T13:29:18Z
...
返回一个由提供的颜色值数组定义的参数值。当传递给 MSL 函数时，它将转换为一对参数。
...
- SY_0025：返回一个参数值，表示着色器所附加的形状或视图的边界矩形，如 IC_0014 所示。对于没有自然边界矩形的着色器（例如，绘制到 `GraphicsContext` 中的滤镜效果），此值未定义。

- **color(_:)**：返回一个表示 `color` 的参数值。当传递给 MSL 函数时，它将转换为 `half4` 值，即目标颜色空间中的预乘颜色。

- **data(_:)**：返回一个由提供的数据值定义的参数值。当传递给 MSL 函数时，它将转换为 `device const void *ptr, int size_in_bytes` 参数对。
...
---
source: https://developer.apple.com/documentation/SwiftUI/Shader/Argument/colorArray(_:)
crawled: 2025-12-04T13:29:18Z
---
