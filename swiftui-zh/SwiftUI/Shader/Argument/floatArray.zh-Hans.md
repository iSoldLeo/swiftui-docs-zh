--- 来源：https://developer.apple.com/documentation/SwiftUI/Shader/Argument/floatArray(_:)

抓取时间：2025-12-04T13:29:25Z
...
返回由提供的浮点数数组定义的参数值。当传递给 MSL 函数时，它将转换为 `device const float *ptr, int count` 参数对。
...
- **boundingRect**：返回一个参数值，表示着色器所附加的形状或视图的边界矩形，如 `float4(x, y, width, height)` 所示。对于没有自然边界矩形的着色器（例如，绘制到 `GraphicsContext` 中的滤镜效果），此值未定义。
...
- **image(_:)**：返回一个由提供的映像定义的参数值。当传递给 MSL 函数时，它将转换为 `texture2d<half>` 值。目前每个 `Shader` 实例仅支持一个图像参数。


---
source: https://developer.apple.com/documentation/SwiftUI/Shader/Argument/floatArray(_:)
crawled: 2025-12-04T13:29:25Z
---
