---
来源： https://developer.apple.com/documentation/SwiftUI/Shape/size(_:)
抓取时间： 2025-12-02T21:41:48Z
---
...
返回代表相同形状的新版本的 self，但要求它从`size` 的矩形创建路径。这不会影响根据形状创建的任何视图的布局属性（例如通过填充）。
...
- **size(width:height:)**：返回代表相同形状的新版本的 self，但要求它从`(width, height)` 大小的矩形中创建路径。这不会影响根据形状创建的任何视图的布局属性（例如通过填充）。
