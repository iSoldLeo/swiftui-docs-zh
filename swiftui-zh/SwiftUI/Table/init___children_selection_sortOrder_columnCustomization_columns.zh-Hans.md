--- 来源：https://developer.apple.com/documentation/SwiftUI/Table/init(_:children:selection:sortOrder:columnCustomization:columns:)
...
创建一个可排序的层级式表格，该表格基于一组可识别的数据以及指向该数据子元素的键路径来计算其行，并支持选择多行。
...
- **children**：指向属性的键路径，该属性的非 `nil` 值表示 `data` 的子项，而 `nil` 值表示层次结构中的叶子行，该叶子行不能拥有子项。
