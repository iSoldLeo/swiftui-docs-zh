--- 来源：https://developer.apple.com/documentation/SwiftUI/Table/init(_:children:sortOrder:columnCustomization:columns:)

抓取时间：2025-12-02T20:59:07Z
...
创建一个可排序的层级式表格，该表格的行数基于一组可识别的数据以及指向该数据子元素的键路径。
...
- **children**：指向属性的键路径，该属性的非 `nil` 值表示 `data` 的子属性，而其 `nil` 值表示层次结构中的叶子行，该叶子行不能拥有子属性。
