--- 来源：https://developer.apple.com/documentation/SwiftUI/Table/init(_:columnCustomization:columns:)

抓取时间：2025-12-01T00:43:03Z
...
创建一个表格，该表格基于一组可识别的数据计算行，并具有可动态自定义的列。
...
表中所有需要进行自定义的列都必须有一个标识符，使用 [customizationID(_:)](../TableColumnContent/customizationID.zh-Hans.md) 指定。

## 创建具有可自定义列的表

- **init(_:selection:columnCustomization:columns:)**：创建一个基于可识别数据集合计算行的表，支持选择多行，并且具有可动态自定义的列。

- **init(_:selection:sortOrder:columnCustomization:columns:)**：创建一个可排序的表，该表基于可识别数据集合计算行，支持选择多行，并且具有可动态自定义的列。

- **init(_:sortOrder:columnCustomization:columns:)**：创建一个可排序的表，该表基于可识别数据集合计算行，并且具有可动态自定义的列。


---
source: https://developer.apple.com/documentation/SwiftUI/Table/init(_:columnCustomization:columns:)
crawled: 2025-12-01T00:43:03Z
---
