--- 来源：https://developer.apple.com/documentation/SwiftUI/Table/init(_:sortOrder:columnCustomization:columns:)

抓取时间：2025-12-01T00:43:05Z
...
创建一个可排序的表格，该表格基于一组可识别的数据计算其行，并且具有可动态自定义的列。
...
- **init(_:columnCustomization:columns:)**：创建一个基于可识别数据集合计算行数且具有动态可自定义列的表格。

- **init(_:selection:columnCustomization:columns:)**：创建一个基于可识别数据集合计算行数、支持选择多行且具有动态可自定义列的表格。

- **init(_:selection:sortOrder:columnCustomization:columns:)**：创建一个可排序的表格，该表格基于一组可识别的数据计算行，支持选择多行，并且具有可动态自定义的列。


---
source: https://developer.apple.com/documentation/SwiftUI/Table/init(_:sortOrder:columnCustomization:columns:)
crawled: 2025-12-01T00:43:05Z
---
