---
来源：https://developer.apple.com/documentation/SwiftUI/TextField/init(值:格式:提示:标签:)
...
创建一个文本字段，将格式样式应用于绑定的值，标签由视图创建器生成。
...
- **value**：要编辑的基础值。
- **format**：`F`类型的格式样式，用于在用户编辑的字符串和`F.FormatInput`类型的底层值之间进行转换。如果`format` 无法执行转换，文本字段将保持值不变。如果用户在无效状态下停止编辑文本，文本字段会将字段文本更新为最后已知的有效值。
- **prompt**：`Text`，可指导用户在文本字段中输入什么内容。
- **label**：视图生成器，可为文本字段生成标签，说明其用途。
...
- **init(_:value:formatter:)**：`V`：创建一个绑定任意类型的实例。
