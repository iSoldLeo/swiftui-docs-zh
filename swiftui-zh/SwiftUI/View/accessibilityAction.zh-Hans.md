--- 来源：https://developer.apple.com/documentation/SwiftUI/View/accessibilityAction(_:_:)
...
向视图添加辅助功能操作。操作允许 VoiceOver 等辅助技术通过调用操作与视图进行交互。
...
例如，以下是如何向视图添加用于撰写新邮件的 `.default` 操作。
...
- **accessibilityAction(named:intent:)**：向视图添加名为 `name` 的辅助功能操作。操作允许辅助技术（例如 VoiceOver）通过调用操作与视图进行交互。执行操作时，将调用 `intent`。

- **accessibilityAdjustableAction(_:)**：向视图添加辅助功能可调节操作。操作允许辅助技术（例如 VoiceOver）通过调用操作与视图进行交互。

- **accessibilityScrollAction(_:)**：向视图添加辅助功能滚动操作。操作允许辅助技术（例如 VoiceOver）通过调用操作与视图进行交互。
