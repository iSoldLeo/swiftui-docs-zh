--- 来源：https://developer.apple.com/documentation/SwiftUI/View/accessibilityActions(_:)

抓取时间：2025-12-02T17:43:47Z
...
操作允许辅助技术（例如 VoiceOver）通过调用操作与视图进行交互。例如，可以通过这种方式向视图添加动态数量的自定义操作。
...
- **accessibilityAction(_:_:)**：向视图添加辅助功能操作。操作允许辅助技术（例如 VoiceOver）通过调用操作与视图进行交互。
