--- 来源：https://developer.apple.com/documentation/SwiftUI/View/accessibilityDragPoint(_:description:)

抓取时间：2025-11-30T21:29:00Z
...
默认情况下，如果辅助视图或其子树包含拖放交互，辅助技术会自动显示这些交互。但是，如果存在多个此类交互，则每个拖放操作都应包含描述，以消除歧义并提供良好的用户体验。
...
- **accessibilityDropPoint(_:description:)**：辅助技术应使用此点结束拖拽交互。

- **accessibilityDropPoint(_:description:isEnabled:)**：辅助技术应使用此点结束拖拽交互。

- **accessibilityDirectTouch(_:options:)**：显式设置此辅助元素是否为直接触摸区域。直接触摸区域会将触摸事件传递给应用程序，而不是通过 VoiceOver 等辅助技术处理。该修饰符接受一个可选的 `AccessibilityDirectTouchOptions` 选项集，用于自定义直接触摸区域的功能。
