--- 来源：https://developer.apple.com/documentation/SwiftUI/View/alert(_:isPresented:actions:message:)

抓取时间：2025-12-02T17:30:26Z
...
当给定条件为真时，显示一个带有消息的提示框，标题使用文本视图。
...
- **message**：返回警报消息的 [ViewBuilder](../ViewBuilder.zh-Hans.md)。
...
警报中的所有操作都会在执行完毕后关闭警报。默认按钮会以更高的优先级显示。您可以通过为默认按钮分配 [defaultAction](../KeyboardShortcut/defaultAction.zh-Hans.md) 键盘快捷键来影响其优先级。
...
- **alert(_:isPresented:presenting:actions:message:)**：使用给定数据生成警报内容，并以文本视图显示标题，从而显示警报消息。

- **alert(isPresented:error:actions:message:)**：出现错误时显示警报消息。


---
source: https://developer.apple.com/documentation/SwiftUI/View/alert(_:isPresented:actions:message:)
crawled: 2025-12-02T17:30:26Z
---
...
- **message**: A [ViewBuilder](../ViewBuilder.zh-Hans.md) returning the message for the alert.
