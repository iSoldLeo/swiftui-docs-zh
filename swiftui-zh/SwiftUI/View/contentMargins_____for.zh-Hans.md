--- 来源：https://developer.apple.com/documentation/SwiftUI/View/contentMargins(_:_:for:)

抓取时间：2025-11-30T21:24:00Z
...
使用此修饰符可以自定义不同类型视图的内容边距。例如，您可以使用此修饰符自定义可滚动视图（例如 [ScrollView](../ScrollView.zh-Hans.md)）的边距。在以下示例中，滚动视图的内容将自动内缩至安全区域，并在前缘和后缘各增加 20 个点。
...
您可以使用 [ContentMarginPlacement](../ContentMarginPlacement.zh-Hans.md) 来定位视图的特定部分进行自定义。例如，使用 [scrollContent](../ContentMarginPlacement/scrollContent.zh-Hans.md) 来内缩 [TextEditor](../TextEditor.zh-Hans.md) 的内容，而不会影响其滚动指示器的内缩。
...
类似地，您可以分别自定义滚动指示器的内缩，而无需考虑滚动内容。当应用可能会裁剪指示器的自定义裁剪形状时，请考虑这样做。
...
应用多个 contentMargins 修饰符时，位置相同的修饰符会覆盖视图层级中位置更高的修饰符。
