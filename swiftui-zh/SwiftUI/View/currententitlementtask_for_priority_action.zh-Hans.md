--- 来源：https://developer.apple.com/documentation/SwiftUI/View/currentEntitlementTask(for:priority:action:)

抓取时间：2025-12-02T17:23:32Z
...
- **productID**：要获取授权的产品 ID。此参数更改时，任务将重新启动。
...
在使用此方法修改的视图显示之前，后台会启动一个任务来获取当前权限。视图显示期间，每当权限发生变化或任务状态发生变化时，该任务都会调用 `action`。

应用内购买的消耗品始终会将 `nil` 传递给 `action`。对于自动续订的订阅，请使用 `subscriptionStatusTask(for:priority:action:)` 获取订阅的完整状态信息。
...
- **onInAppPurchaseStart(perform:)**：添加一个操作，用于在用户点击本视图中的 StoreKit 视图上的购买按钮时执行。
