--- 来源：https://developer.apple.com/documentation/swiftui/view/dialogsuppressiontoggle(_:issuppressed:)

抓取时间：2025-12-01T10:51:47Z
...
允许用户在 macOS 上通过自定义抑制消息来抑制 `self` 中显示的对话框和警告。其他平台未使用。
...
在 macOS 上应用对话框抑制功能会在对话框上添加一个开关，允许用户请求不再显示该提示。通常，对话框是否被抑制的信息存储在 `AppStorage` 中，并用于决定将来是否显示该对话框。
...
- **dialogSuppressionToggle(isSuppressed:)**：允许用户屏蔽 `self` 中显示的对话框和警报，macOS 系统默认显示屏蔽消息。其他平台未使用此功能。

- **dialogSuppressionToggle(isSuppressed:)**：允许用户使用自定义屏蔽消息屏蔽警报。

- **dialogSuppressionToggle(_:isSuppressed:)**：允许用户使用自定义屏蔽消息屏蔽警报。


---
source: https://developer.apple.com/documentation/swiftui/view/dialogsuppressiontoggle(_:issuppressed:)
crawled: 2025-12-01T10:51:47Z
---
