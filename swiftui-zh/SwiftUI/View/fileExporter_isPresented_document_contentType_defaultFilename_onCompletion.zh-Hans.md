--- 来源：https://developer.apple.com/documentation/SwiftUI/View/fileExporter(isPresented:document:contentType:defaultFilename:onCompletion:)

抓取时间：2025-12-02T17:30:43Z
...
提供一个系统接口，用于将存储在值类型（例如结构体）中的文档导出到磁盘上的文件。
...
- **defaultFilename**：如果提供，则为导出文件的默认名称，用户可以在导出前对其进行编辑。

- **onCompletion**：操作成功或失败时将调用的回调函数。

## 讨论

为了显示界面，`isPresented` 必须为 `true`，且 `document` 不能为 `nil`。操作完成后，在调用 `onCompletion` 之前，`isPresented` 将被设置为 `false`。如果用户取消操作，`isPresented` 将被设置为 `false`，并且不会调用 `onCompletion`。
