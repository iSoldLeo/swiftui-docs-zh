--- 来源：https://developer.apple.com/documentation/SwiftUI/View/fileExporter(isPresented:documents:contentTypes:onCompletion:onCancellation:)

抓取时间：2025-11-30T21:15:47Z
...
显示一个系统对话框，允许用户将符合 `FileDocument` 标准的文档集合导出到磁盘上的文件。
...
要显示对话框，`isPresented` 必须为 `true`。操作完成后，在调用 `onCompletion` 之前，`isPresented` 将被设置为 `false`。如果用户取消操作，`isPresented` 将被设置为 `false`，并调用 `onCancellation`。
...
- **fileExporter(isPresented:item:contentTypes:defaultFilename:onCompletion:onCancellation:)**：提供一个系统接口，允许用户将 `Transferable` 项导出到磁盘上的文件。

- **fileExporter(isPresented:items:contentTypes:onCompletion:onCancellation:)**：提供一个系统界面，允许用户将一组项目导出到磁盘上的文件。

- **fileExporterFilenameLabel(_:)**：在 macOS 上，配置 `fileExporter`，为文件名字段添加标签。
