--- 来源：https://developer.apple.com/documentation/SwiftUI/View/imagePlaygroundSheet(isPresented:concept:sourceImage:onCompletion:onCancellation:)

抓取时间：2025-11-30T21:09:04Z
...
- **sourceImage**：指定一个现有图像作为创建新图像的源输入。查看此工作表的用户可以覆盖您提供的图像，并在创建界面中选择不同的图像和概念。如果您未提供起始图像，系统将仅使用 `concepts` 参数的内容创建新图像。

- **onCompletion**：用于接收已创建图像的块。此块没有返回值，并接收以下参数：

- **onCancellation**：用于处理用户在未选择任何图像的情况下退出创建界面时取消图像创建过程的块。执行此块后，系统将自动关闭工作表。
