--- 来源：https://developer.apple.com/documentation/SwiftUI/View/inAppPurchaseOptions(_:)

抓取时间：2025-12-02T17:24:04Z
...
添加一个函数，在从 StoreKit 视图发起购买之前调用该函数，并提供一组购买选项。
...
- **options**：系统在处理购买之前调用此函数，并将要购买的产品作为参数提供。返回要添加到购买中的一组购买选项。

## 说明

此视图中的应用内商店会将任何默认购买选项添加到您返回的集合中，并使用结果来配置购买。如果您只想对应用内购买做出响应，而无需添加购买选项，可以使用 [onInAppPurchaseStart(perform:)](onInAppPurchaseStart_perform.zh-Hans.md) 添加操作。

您可以通过为操作提供 `nil` 来移除父视图可能添加的任何选项。这将使用默认的购买选项集。
...
- **onInAppPurchaseCompletion(perform:)**：添加一个操作，当用户从此视图内的 StoreKit 视图发起的购买操作完成时执行。

- **onInAppPurchaseStart(perform:)**：添加一个操作，当用户点击此视图内 StoreKit 视图上的购买按钮时执行。
...
In-app stores within this view will add any default purchase options to the set you return, and use the result for configuring the purchase. If you just want to react to in-app purchases beginning without adding purchase options, you can add an action with [onInAppPurchaseStart(perform:)](onInAppPurchaseStart_perform.zh-Hans.md).
