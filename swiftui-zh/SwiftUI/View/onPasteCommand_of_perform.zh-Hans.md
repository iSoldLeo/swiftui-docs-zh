--- 来源：https://developer.apple.com/documentation/SwiftUI/View/onPasteCommand(of:perform:)

抓取时间：2025-12-02T17:42:36Z
...
- **payloadAction**：粘贴命令触发时要执行的操作。操作闭包的参数包含剪贴板中的项目，其类型由您在 `supportedContentTypes` 参数中指定。
...
将统一类型标识符数组传递给 `supportedContentTypes` 参数。将优先级较高的类型放在数组的开头附近。`action` 闭包接收到的剪贴板项目具有源支持的所有类型中优先级最高的类型。

例如，如果您的应用可以处理纯文本和富文本，但您更倾向于富文本，请将富文本类型放在数组的开头。如果在执行粘贴操作时富文本可用，`action` 闭包会传递该富文本。

## 使用项目提供程序复制项目

- **onCopyCommand(perform:)**：添加一个操作，用于响应系统的“复制”命令。

- **onCutCommand(perform:)**：添加一个操作，用于响应系统的“剪切”命令。

- **onPasteCommand(of:validator:perform:)**：添加一个操作，用于响应系统的“粘贴”命令（针对您已验证的项目）。


---
source: https://developer.apple.com/documentation/SwiftUI/View/onPasteCommand(of:perform:)
crawled: 2025-12-02T17:42:36Z
---
