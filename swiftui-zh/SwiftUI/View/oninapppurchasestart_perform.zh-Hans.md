--- 来源：https://developer.apple.com/documentation/swiftui/view/oninapppurchasestart(perform:)
...
添加一个操作，当用户点击此视图中 StoreKit 视图上的购买按钮时，该操作将执行。
...
- **action**：要执行的操作，要购买的产品作为参数提供。

## 说明

您可以通过为操作提供 `nil` 来移除祖先视图可能添加的任何操作。
...
- **inAppPurchaseOptions(_:)**：添加一个函数，在从本视图中的 StoreKit 视图发起购买之前调用该函数，并提供一组购买选项。

- **manageSubscriptionsSheet(isPresented:subscriptionGroupID:)**

- **onInAppPurchaseCompletion(perform:)**：添加一个操作，当从本视图中的 StoreKit 视图发起的购买完成时执行。
