--- 来源：https://developer.apple.com/documentation/swiftui/view/onkeypress(_:phases:action:)

抓取时间：2025-12-03T06:44:34Z
---
...
- **phases**：要匹配的按键阶段（`.down`、`.up` 和 `.repeat`）。
...
SwiftUI 会针对指定的事件阶段执行操作。

## 响应键盘输入

- **onKeyPress(_:action:)**：当用户在视图获得焦点时按下硬件键盘上的某个键，则执行该操作。

- **onKeyPress(phases:action:)**：当用户在视图获得焦点时按下硬件键盘上的任意键，则执行该操作。

- **onKeyPress(characters:phases:action:)**：当用户在视图获得焦点时按下硬件键盘上的一个或多个键，则执行该操作。

- **onKeyPress(keys:phases:action:)**：当用户在视图获得焦点时按下硬件键盘上的一个或多个键，则执行相应操作。
