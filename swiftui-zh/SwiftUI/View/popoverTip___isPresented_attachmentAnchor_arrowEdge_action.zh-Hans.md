--- 来源：https://developer.apple.com/documentation/SwiftUI/View/popoverTip(_:isPresented:attachmentAnchor:arrowEdge:action:)

抓取时间：2025-11-30T21:10:12Z
...
- **isPresented**：一个绑定，当显示提示时会自动更新为 true。可以更改此值来临时隐藏或显示当前可显示的提示。如果此值为 `nil`，则弹出框将根据提示的状态和显示规则自动关闭。

- **attachmentAnchor**：定义弹出框连接点的定位锚点。默认值为 bounds。

- **arrowEdge**：定义弹出框箭头位置的 attachmentAnchor 边缘。默认情况下，系统会选择弹出框箭头的最佳方向。
