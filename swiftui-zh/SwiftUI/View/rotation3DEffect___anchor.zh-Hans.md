--- 来源：https://developer.apple.com/documentation/swiftui/view/rotation3deffect(_:anchor:)

抓取时间：2025-12-03T06:27:34Z
...
此修改器可旋转视图内容，而不会改变视图的框架。以下代码使用视图中心的默认锚点，显示一个绕 y 轴旋转 45° 的 3D 模型：
...
在动画过程中，此修改器使用球面线性插值，可生成更自然的动画，但不支持超过 360 度的旋转。要指定超过 360 度的角度，请考虑使用 `View/rotation3DEffect(_:axis:anchor:)-4enag`。
...
- **rotationEffect(_:anchor:)**：围绕指定点，在二维方向上旋转视图的渲染输出。

- **rotation3DEffect(_:axis:anchor:anchorZ:perspective:)**：将视图内容渲染成绕指定轴在三维空间中旋转一周的样子。

- **perspectiveRotationEffect(_:axis:anchor:anchorZ:perspective:)**：将视图内容渲染成绕指定轴在三维空间中旋转一周的样子。
