--- 来源：https://developer.apple.com/documentation/SwiftUI/View/scaledToFill()

抓取时间：2025-12-02T17:37:05Z
...
一个缩放此视图以填充其父视图的视图，并保持此视图的宽高比。

## 讨论

使用 `scaledToFill()` 可将此视图缩放以填充其父视图，同时保持视图缩放时的宽高比：
...
此方法等效于调用 [aspectRatio(_:contentMode:)](aspectRatio___contentMode.zh-Hans.md)，并指定 `nil` aspectRatio 和 [fill](../ContentMode/fill.zh-Hans.md) content mode。

## 缩放、旋转或变换视图

- **scaledToFit()**：缩放此视图以适应其父视图。

- **scaleEffect(_:anchor:)**：相对于锚点，按给定的水平和垂直方向缩放此视图的渲染输出。
...
This method is equivalent to calling [aspectRatio(_:contentMode:)](aspectRatio___contentMode.zh-Hans.md) with a `nil` aspectRatio and a content mode of [fill](../ContentMode/fill.zh-Hans.md).
