---
来源： https://developer.apple.com/documentation/swiftui/textinputformattingcontrolplacement
抓取时间： 2025-12-05T22:23:39Z
---
...
- **TextInputFormattingControlPlacement.Set**：一组系统文本格式控件。

## 管理文本输入

- **autocorrectionDisabled(_:)**：设置是否禁用此视图的自动更正功能。
- **autocorrectionDisabled**：布尔值，用于确定视图层次结构是否启用了自动更正功能。
- **keyboardType(_:)**：设置此视图的键盘类型。
- **scrollDismissesKeyboard(_:)**：配置可滚动内容与软件键盘交互的行为。
- **textContentType(_:)**：设置此视图的文本内容类型，当用户在 macOS 上输入文本时，系统会使用该类型提供建议。
...
- **textInputSuggestions(_:content:)**：配置此视图的文本输入建议：配置此视图的文本输入建议。
...
- **TextInputFormattingControlPlacement.Set**: A set of system text formatting controls.
